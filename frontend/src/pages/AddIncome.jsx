...
import { useAddIncomeMutation } from "../store/features/incomeSlice";
...
const AddIncome = () => {  
  ...
  const handleSubmit = (e) => {
    ...
    const button = document.getElementById("button-add-income")
    button.disabled = true; // deshabilitar el botón para evitar múltiples envíos

    // llamada a la mutación para agregar el ingreso
    addIncome(incomeData)
      .then(() => {
        toast.success("Ingreso agregado correctamente. Redirigiendo a la sección 'Reportes'", {
  ...
  return (
    <main className="w-full overflow-y-auto p-4 md:p-6 h-screen md:text-lg max-w-[1200px] mx-auto">
      <h2 className="text-xl md:text-2xl font-medium text-center pb-4">Agregar ingreso</h2>
      <form onSubmit={handleSubmit} className="p-4 bg-white rounded-xl text-[#01578f] flex flex-col gap-2">
        <div className="flex flex-col w-full">
          ...
          <input type="date" name="income-date" className="border-1 rounded-md px-1 py-0.5"/>
        ...
        <div className="flex flex-col w-full">
          ...
          <textarea rows={3} name="income-description" className="border-1 resize-none rounded-md px-1 py-0.5"/>
        ...
        <div className="flex flex-col w-full">
          ...
          <select name="income-categories" defaultValue="" className="border-1 rounded-md px-1 py-0.5 focus:outline-none">
        ...
        <div className="flex flex-col w-full">
          ...
          <select name="income-payment" defaultValue="" className="border-1 rounded-md px-1 py-0.5 focus:outline-none">
            <option value="" disabled >
        ...
        <div className="flex flex-col md:flex-row gap-2 justify-center items-center w-full">
          <button id="button-add-income" type="submit" className="font-semibold bg-[#01578f] text-white mt-3 py-1 rounded-md w-full">Guardar</button>
